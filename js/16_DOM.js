...
document.querySelector("#btn1").addEventListener("click",()=>{
    ...
    // 1. li1의 부모 노드 찾기(요소.parentNode) + 부모 노드 배경색 변경
    // 자식은 여럿이 가능하기에 Nodes 지만 부모는 단수인 Node를 씀

    li1.parentNode.style.backgroundColor = "orange";

    // 2. 첫 번째 자식 노드 찾기(요소.firstChild)
    // -> #test(ul태그)의 첫 번째 자식 노드 찾기
    console.log(test.firstChld);
    ...
    // -> #test(ul태그)의 자식 노드 중 3번째 li 태그를 찾아서 배경색 pink로 지정하기
